...
class OptimisticFLock
{
	 ...
	 */
	function ofl_file_get_contents($f_name, &$ofl_lock, &$err_code)
	{
	 ...
	 * @return status code
	 */
	function ofl_file_put_contents($f_name, $f_contents, &$ofl_lock, $ofl_force = false)
	{
	 ...
	 */
	function ofl_parse_ini_file($f_name, $ini_structured = false, &$ofl_lock, &$err_code)
	{
			// check file exists and readable
		if(file_exists($f_name) && is_readable($f_name))
		{
	 ...
	 * @return status code
	 */
	function ofl_touch($f_name, &$ofl_lock)
	{
		// exclusive flock index
		$this->__tm_index_flock(LOCK_EX);
	 ...
	 */
	function ofl_unlink($f_name, &$ofl_lock, $ofl_force = false)
	{
	 ...
	 */
	function __tm_index_flock($operation)
	{
	 ...
	 */
	function __tm_read_or_insert_ofl($f_name)
	{
	 ...
	 */
	function __tm_insert_or_update_ofl($f_name)
	{
	 ...
	 */
	function __tm_delete_ofl($f_name)
	{
		if(!is_array($this->cache_tm_index))
		{
			$this->__tm_index_load();
